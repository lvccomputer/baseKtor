...
fun String.decodeBase64(): String = runCatching {
    val decoded = toByteArray().decodeBase64()
    runCatching { String(decoded, charset = Charsets.UTF_8) }.getOrElse {
        runCatching { String(decoded, charset = Charsets.ISO_8859_1) }.getOrElse {
            String(decoded.filter { it.toInt() > 0 }.toByteArray())
        }
    }
}.getOrNull() ?: this
...
fun ByteArray.encodeBase64(): ByteArray {
    ...
    while (position < this.size) {
        ...
        for (i in 0 until 4 - padding) {
            ...
            output.add(table[c].code.toByte())
            b = b shl 6
        }
        position += 3
    }
